...
export function writeContextFile(workspacePath: string, context: string): void {
  ...
  const rulesFile = join(rulesDir, 'pilot-memory-context.mdc');
...
The following context is from pilot-memory, a persistent memory system that tracks your coding sessions.

${context}

---
*Updated after last session. Use pilot-memory's MCP search tools for more detailed queries.*
`;
...
export function readContextFile(workspacePath: string): string | null {
  const rulesFile = join(workspacePath, '.cursor', 'rules', 'pilot-memory-context.mdc');
...
/**
 * Configure pilot-memory MCP server in Cursor's mcp.json
...
/**
 * Remove pilot-memory MCP server from Cursor's mcp.json
...
export function removeMcpConfig(mcpJsonPath: string): void {
  ...
  try {
    ...
    if (config.mcpServers && config.mcpServers['pilot-memory']) {
      delete config.mcpServers['pilot-memory'];
      writeFileSync(mcpJsonPath, JSON.stringify(config, null, 2));
    }
  } catch (e) {
