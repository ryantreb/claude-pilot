/**
 * CursorHooksInstaller - Cursor IDE integration for pilot-memory
 *
...
export function configureCursorMcp(target: CursorInstallTarget): number {
  ...
  try {
    ...
    // Add pilot-memory MCP server
    config.mcpServers['pilot-memory'] = {
      command: 'node',
      args: [mcpServerPath]
    };
...
export async function installCursorHooks(_sourceDir: string, target: CursorInstallTarget): Promise<number> {
  console.log(`\nInstalling Pilot Memory Cursor hooks (${target} level)...\n`);
...
Next steps:
  1. Start pilot-memory worker: pilot-memory start
  2. Restart Cursor to load the hooks
  3. Check Cursor Settings → Hooks tab to verify

Context Injection:
  Context from past sessions is stored in .cursor/rules/pilot-memory-context.mdc
...
async function setupProjectContext(targetDir: string, workspaceRoot: string): Promise<void> {
  ...
  if (!contextGenerated) {
    ...
    const rulesFile = path.join(rulesDir, 'pilot-memory-context.mdc');
    const placeholderContent = `---
alwaysApply: true
description: "Pilot Memory context from past sessions (auto-updated)"
---
...
Use pilot-memory's MCP search tools for manual memory queries.
`;
...
export function uninstallCursorHooks(target: CursorInstallTarget): number {
  console.log(`\nUninstalling Pilot Memory Cursor hooks (${target} level)...\n`);
  ...
  try {
    ...
    if (target === 'project') {
      const contextFile = path.join(targetDir, 'rules', 'pilot-memory-context.mdc');
...
export function checkCursorHooksStatus(): number {
  console.log('\nPilot Memory Cursor Hooks Status\n');
  ...
  for (const loc of locations) {
    ...
    if (existsSync(hooksJson)) {
      ...
      if (loc.name === 'Project') {
        const contextFile = path.join(loc.dir, 'rules', 'pilot-memory-context.mdc');
        if (existsSync(contextFile)) {
  ...
  if (!anyInstalled) {
    console.log('No hooks installed. Run: pilot-memory cursor install\n');
  }
...
export async function handleCursorCommand(subcommand: string, args: string[]): Promise<number> {
  switch (subcommand) {
    ...
    default: {
      console.log(`
Pilot Memory Cursor Integration

Usage: pilot-memory cursor <command> [options]
...
Examples:
  ...
  pilot-memory cursor install user         # Install globally for user
  pilot-memory cursor uninstall            # Remove from current project
  pilot-memory cursor status               # Check if hooks are installed

For more info: https://github.com/customable/pilot-memory#readme
      `);
      return 0;
    }
  }
