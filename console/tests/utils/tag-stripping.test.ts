 ...
 *
 * Tests the dual-tag privacy system for <private> and <pilot-memory-context> tags.
...
describe('Tag Stripping Utilities', () => {
  ...
  describe('stripMemoryTagsFromPrompt', () => {
    describe('basic tag removal', () => {
      ...
      it('should strip single <pilot-memory-context> tag', () => {
        const input = 'public content <pilot-memory-context>injected context</pilot-memory-context> more public';
        const result = stripMemoryTagsFromPrompt(input);
        expect(result).toBe('public content  more public');
      });

      it('should strip both tag types in mixed content', () => {
        const input = '<private>secret</private> public <pilot-memory-context>context</pilot-memory-context> end';
        const result = stripMemoryTagsFromPrompt(input);
    ...
    describe('multiple tags handling', () => {
      ...
      it('should strip multiple <pilot-memory-context> blocks', () => {
        const input = '<pilot-memory-context>ctx1</pilot-memory-context><pilot-memory-context>ctx2</pilot-memory-context> content';
        const result = stripMemoryTagsFromPrompt(input);
      ...
      it('should handle many interleaved tags', () => {
        let input = 'start';
        for (let i = 0; i < 10; i++) {
          input += ` <private>p${i}</private> <pilot-memory-context>c${i}</pilot-memory-context>`;
        }
        input += ' end';
        const result = stripMemoryTagsFromPrompt(input);
        // Tags are stripped but spaces between them remain
        expect(result).not.toContain('<private>');
        expect(result).not.toContain('<pilot-memory-context>');
    ...
    describe('empty and private-only prompts', () => {
      ...
      it('should return empty string for entirely context-tagged prompt', () => {
        const input = '<pilot-memory-context>all is context</pilot-memory-context>';
        const result = stripMemoryTagsFromPrompt(input);
        expect(result).toBe('');
      });
      ...
      it('should handle whitespace-only after stripping', () => {
        const input = '<private>content</private>   <pilot-memory-context>more</pilot-memory-context>';
        const result = stripMemoryTagsFromPrompt(input);
        expect(result).toBe('');
      });
    });
...
secret
      ...
      it('should strip multiline content within <pilot-memory-context> tags', () => {
        const input = `start
<pilot-memory-context>
# Recent Activity
- Item 1
- Item 2
</pilot-memory-context>
finish`;
  ...
  describe('stripMemoryTagsFromJson', () => {
    describe('JSON content stripping', () => {
      ...
      it('should strip pilot-memory-context tags from JSON', () => {
        const jsonContent = JSON.stringify({
          data: '<pilot-memory-context>injected</pilot-memory-context> real data'
        });
        const result = stripMemoryTagsFromJson(jsonContent);
        const parsed = JSON.parse(result);
      ...
      it('should handle tool_response with tags', () => {
        const toolResponse = {
          output: 'result <pilot-memory-context>context data</pilot-memory-context>',
          status: 'success'
        };
